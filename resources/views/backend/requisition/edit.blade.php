...
@section('title')
Requisition   Update information | {{ $ins_name }}
@endsection
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Update Requisition </h4>
...
<form method="POST" action="{{ route('admin.requisition_information.update') }}">
    @csrf

    <input type="hidden" value="{{ $r_list->id }}" name="id" />
<div class="row">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <div class="input-group" id="datepicker2">
                            <input type="text" name="request_date" value="{{ $r_list->request_date }}" class="form-control" placeholder="dd M, yyyy"
...
@if($Auth::guard('admin')->user()->id == 1)
                ...
                <div class="form-group row mt-2">
                    ...
                    <div class="col-sm-8">
                        ...
                        <option value="{{ $all_user->id }}" {{ $r_list->admin_id == $all_user->id  ? 'selected':''  }}>{{ $all_user->name }}({{ $all_user->position }})</option>
                        @endforeach
                    </select>
                    </div>
                </div>
                ...
                <div class="form-group row mt-2">
                    ...
                    <div class="col-sm-8">
                        <select name="request_type" id="" class="form-control">
                            <option value="Normal" {{ $r_list->request_type == 'Normal' ? 'selected':''  }}>Normal</option>
                            <option value="Emergency" {{ $r_list->request_type == 'Emergency' ? 'selected':''  }}>Emergency</option>
                        </select>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table align-middle table-nowrap table-check" id="dynamicAddRemove">
                                        ...
                                        <tbody>
                                            @foreach($r_product_list as $key=>$all_r_product_list)

                                            @if(($key+1) == 1)
                                        <tr>

                                            <td style="width:400px">
                                                <select class=" form-control main_product_id select2" name="nmain_product_id[]" id="main_product_id{{ $key+50000 }}" >
                                                    <option value="0">Please Select</option>
                                                    @foreach($product_names as $main_product_list_all_print)
                                                    <option value="{{ $main_product_list_all_print->id }}" {{ $all_r_product_list->product_id == $main_product_list_all_print->id  ? 'selected':'' }}>{{ $main_product_list_all_print->name }}</option>
                                                    @endforeach
                                                </select>
                                            </td>

                                            <td><input type="number" min="1" class="form-control p_quantity" value="{{ $all_r_product_list->quantity  }}" name="p_quantity[]" id="p_quantity{{ $key+50000 }}" placeholder="Quantity"></td>
