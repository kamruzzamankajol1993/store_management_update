...
<div class="row">
                        <div class="col-sm-6">
                            <div class="float-right d-md-block">
                                <div class="dropdown">
                                @if (Auth::guard('admin')->user()->can('sub_category_add'))
<button class="btn btn-primary dropdown-toggle waves-effect  btn-sm waves-light" type="button" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg">
                                        <i class="far fa-calendar-plus  mr-2"></i> Add New Sub Category
                                    </button>
@endif
                                </div>
                            </div>
    ...
    style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <td>{{ $user->sub_cate }}</td>
                                    ...
                                    <td>
                                      ...
                                      @if (Auth::guard('admin')->user()->can('sub_category_update'))
                                                ...
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="{{ route('admin.sub_category_information.update') }}" method="POST" enctype="multipart/form-data">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="form-group col-md-6 col-sm-12">
                                                                ...
                                                                <select class="form-control form-control-sm" id="email" name="sub_cate">
                                                                    <option>--Please Select--</option>
                                                                    @foreach($users1 as $all_cat)

                                                                    <option value="{{ $all_cat->name }}"  {{ $user->sub_cate == $all_cat->name  ? 'selected':'' }}>{{ $all_cat->name }}</option>
...
{{-- <button type="button" class="btn btn-primary waves-light waves-effect  btn-sm" onclick="window.location.href='{{ route('admin.users.view',$user->id) }}'"><i class="fa fa-eye"></i></button> --}}
                                  ...
                                  @if (Auth::guard('admin')->user()->can('sub_category_delete'))
...
<button   type="button" class="btn btn-danger waves-light waves-effect  btn-sm" onclick="deleteTag({{ $user->id}})" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.sub_category_information.delete',$user->id) }}" method="POST" style="display: none;">
...
@endforeach
  ...
  <div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Add New Sub Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <div class="modal-body">
                <form class="custom-validation" action="{{ route('admin.sub_category_information.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                      <div class="row">
